...
function PropertyCard({ property }) {
  ...
  return (
    <motion.article
      ref={cardRef}
      className="group flex h-full min-h-0 flex-col overflow-hidden rounded-xl sm:rounded-2xl border border-charcoal/[0.08] bg-warm shadow-[0_2px_16px_-4px_rgba(28,28,28,0.08)]"
    ...
    >
      ...
      >
        ...
        <div
          className="relative h-[132px] w-full flex-shrink-0 overflow-hidden border-b border-gold/20 bg-gradient-to-br from-white via-warm to-sand/80 sm:h-[220px] lg:h-[300px]"
        ...
        {/* Body */}
        <div
          className="flex flex-1 flex-col px-3 pb-3 pt-3 sm:px-6 sm:pb-6 sm:pt-6 transition-colors duration-500 group-hover:bg-warm/[0.97] lg:px-8 lg:pb-8 lg:pt-8"
          style={{ transform: 'translateZ(6px)' }}
        >
          <div className="mb-1 sm:mb-2 min-h-[1.1rem] sm:min-h-[2.75rem] lg:min-h-[3.25rem]">
            <h3 className="m-0 font-serif text-[10px] sm:text-xl text-charcoal transition-colors duration-500 ease-out lg:text-2xl group-hover:text-gold whitespace-nowrap overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {property.name}
            </h3>
          </div>

          <p className="mb-2 sm:mb-4 flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-sm text-soft whitespace-nowrap overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <p className="hidden sm:block mb-6 min-h-[3.25rem] leading-relaxed text-charcoal line-clamp-2">
            {property.tagline}
          </p>
